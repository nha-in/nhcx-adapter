...
//	nhcx-adapter ledger follow [--tail N] [filters]
...
Usage:
  nhcx-adapter serve    [--config FILE] [--no-tui] [--skip-checks] [--no-banner] [--no-update-check]
                                                                 ...
                                                                 serves the browser panel when panel.password is set)
  ...
  nhcx-adapter ledger follow [same filters] [--tail N] [--json]  watch messages arrive, live and coloured
...
func cmdLedger(args []string) error {
	...
	var (
		...
		tail        = fs.Int("tail", 10, "follow: show this many recent messages before waiting")
	)
	...
	case "list":
		...
		return nil
	case "follow", "tail":
		return followLedger(store, ledger.Query{Direction: *direction, Entity: *entity, Kind: *kind, Status: *status,
			CorrelationID: *corr, WorkflowID: *workflow, Participant: *participant}, *tail, *asJSON)
	case "show":
		if len(positional) == 0 {
	...
	return fmt.Errorf("unknown ledger command %q (list, follow, show, thread, stats, clear)", sub)
}

// followLedger prints the last few messages and then every one that lands,
// as they land — the traffic lines "serve" prints, from a second terminal or
// from a machine where the adapter runs as a service and its log goes
// somewhere else.
//
// It reads the ledger files, not the server: nothing has to be running here,
// nothing is asked of the process that is, and the same command works
// against a ledger that is only being written to (a live adapter) or one
// that is finished with (yesterday's, copied off a box).
func followLedger(store *ledger.Store, q ledger.Query, tail int, asJSON bool) error {
	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer stop()

	emit := func(sm ledger.Summary) { fmt.Println(ledgerLine(sm)) }
	if asJSON {
		enc := json.NewEncoder(os.Stdout)
		enc.SetEscapeHTML(false)
		emit = func(sm ledger.Summary) { _ = enc.Encode(sm) } // one JSON object per line
	}

	// The floor is the newest id on record, filtered or not: an entry
	// written between opening the ledger and starting to watch it must not
	// be printed twice.
	var after string
	if newest := store.List(ledger.Query{Limit: 1}); len(newest) > 0 {
		after = newest[0].ID
	}
	if tail > 0 {
		backfill := q
		backfill.Limit = tail
		items := store.List(backfill)
	...
	if !asJSON {
		st := store.Stats()
		fmt.Fprintf(os.Stderr, "%s %s · %d messages on record · %s\n",
			style.Dim("following"), style.Key(st.Dir), st.Total, style.Dim("ctrl-c to stop"))
	}

	err := store.Follow(ctx, ledger.FollowOptions{After: after, Query: q}, emit)
...
// ledgerLine renders one recorded message as the coloured traffic line the
// server logs, so a ledger read back looks like the traffic did going past.
func ledgerLine(sm ledger.Summary) string {
	peer := sm.Recipient
	if sm.Direction == ledger.In {
		peer = sm.Sender
	}
	m := logfmt.Message{
		Time: sm.CreatedAt, ID: sm.ID, Direction: sm.Direction, Path: sm.Path, Peer: peer, Status: sm.Status,
		PeerStatus: sm.PeerStatus, TookMs: sm.DurationMs, Format: sm.Format, Redelivery: sm.Redelivery,
		CorrelationID: sm.CorrelationID,
	}
	if sm.Error != nil {
		m.Error = sm.Error.Code
	}
	return m.Render()
}
