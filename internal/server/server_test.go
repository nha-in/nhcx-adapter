...
// newFixture's optional mutate hooks run on the parsed config before the
// adapter is built, so a test can add to it (a fan-out target, say) without
// its own copy of the whole setup.
func newFixture(t *testing.T, apiKey string, mutate ...func(*config.Config)) *fixture {
	t.Helper()
	...
	mux.HandleFunc("POST /hcx/v1/{path...}", func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasSuffix(r.URL.Path, "/drop") {
			// The request was read; the connection dies before any answer.
			...
			panic("cannot drop the connection")
		}
	...
	cfgJSON := fmt.Sprintf(`{
	  ...
	  "panel": {"password": %q},
	  "log": {"level": "error"}
	}`, apiKey, pemPrivate(t, f.self), f.callback.URL+"/hook", f.callback.URL+"/claims-only", f.abdm.URL+"/hcx/v1", f.abdm.URL+"/registry", f.abdm.URL+"/sessions", t.TempDir(), panelPassword)
...
func TestOutboundFailures(t *testing.T) {
	...
	// A connection dropped after the request was written is a failure that
	// still names the ids the message went out under: NHCX may have taken
	// it, and the caller needs the correlation to match the answer.
	status, out = f.post("/out/v1/claim/drop", `{"recipient":"1000004805","fhir":`+bundle+`}`, nil)
	e, _ := out["error"].(map[string]any)
	if status != http.StatusBadGateway || out["ok"] != false || e["retryable"] != true {
		t.Errorf("dropped connection: status %d %v", status, out)
	}
	for _, key := range []string{"ledger_id", "txn_id", "correlation_id", "api_call_id", "request_id"} {
		if id, _ := out[key].(string); id == "" {
			t.Errorf("dropped connection: %s missing from %v", key, out)
		}
	}
	if hdrs, _ := out["headers"].(map[string]any); hdrs["x-hcx-correlation_id"] != out["correlation_id"] {
		t.Errorf("dropped connection: headers %v", out["headers"])
	}
...
func TestInboundFailures(t *testing.T) {
	...
	if status, _ := f.post("/in/v1/claim/on_submit", strings.Repeat("x", 101<<20), nil); status != http.StatusRequestEntityTooLarge {
