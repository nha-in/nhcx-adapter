...
var sections = []section{
	...
	{"Panel (browser console)", []field{
		{path: "panel.password", label: "Password", kind: kSecret,
			help: "Set one to serve the operator panel at /panel; blank keeps it off. At least 8 characters. ${VAR} reads it from the environment."},
		{path: "panel.path", label: "Path", kind: kText, help: "Where the panel is served. Default /panel."},
		{path: "panel.sessionHours", label: "Session (hours)", kind: kInt, help: "How long a login lasts. Default 12. Restarting the adapter ends every session."},
	}},
	{"TLS & limits", []field{
		...
		{path: "maxBodyBytes", label: "Max body (bytes)", kind: kInt, help: "Default 104857600 (100 MiB)."},
