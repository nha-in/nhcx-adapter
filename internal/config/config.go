...
type Callback struct {
	...
	// Also lists further callbacks the same delivery is posted to, for a
	// participant code shared by more than one backend (two EMRs behind one
	// registration, say). Every target must accept the message before the
	// delivery counts — a refusal anywhere makes NHCX redeliver to all of
	// them, so each backend has to stay idempotent on x-hcx-api_call_id.
	// Unset fields fall back per entry: appendPath to true, timeoutSeconds
	// to the primary's. An entry may not nest an also of its own.
	Also []Callback `json:"also,omitempty"`
}
...
// Panel is the operator console: a small web page on this same listener
// that shows the ledger as it happens and can send a message, look up a
// participant, refresh the session — behind a password.
//
// It is off until a password is set. An adapter reachable from a network is
// reachable at this path too, so the page is deliberately something you turn
// on rather than something you turn off.
type Panel struct {
	// Enabled forces the panel on or off. Unset means "on when a password
	// is set" — see Config.PanelEnabled.
	Enabled *bool `json:"enabled,omitempty"`
	// Password is what the login form asks for. ${ENV} keeps it out of the
	// file. At least 8 characters; there is no user name.
	Password string `json:"password,omitempty"`
	// Path is where the panel is served, default /panel.
	Path string `json:"path,omitempty"`
	// SessionHours is how long a login lasts, default 12. Restarting the
	// adapter ends every session regardless: the cookie key is minted at
	// startup and never written down.
...
type Config struct {
	...
	// MaxBodyBytes caps request bodies on both surfaces. Default 100 MiB: a
	// pre-authorisation or claim dossier carries its documents inline as
	// base64, and a scanned file set is tens of megabytes.
...
func (c *Config) applyDefaults() {
	...
	c.Panel.Path = "/" + strings.Trim(strings.TrimSpace(c.Panel.Path), "/")
	if c.Panel.Path == "/" {
		c.Panel.Path = "/panel"
	}
	if c.Panel.SessionHours <= 0 {
		c.Panel.SessionHours = 12
	}
	...
	cleanRoutes(c.Callback.Routes)
	for i := range c.Callback.Also {
		cleanRoutes(c.Callback.Also[i].Routes)
	}
	if c.Log.Level == "" {
		c.Log.Level = "info"
	}
...
func normalizeParticipant(p *Participant) {
	...
	cleanRoutes(p.Callback.Routes)
	for i := range p.Callback.Also {
		cleanRoutes(p.Callback.Also[i].Routes)
	}
}

// cleanRoutes trims the slashes off route keys so a "/v1/x/" spelling matches
// the cleaned inbound path.
func cleanRoutes(routes map[string]string) {
...
func (c *Config) ValidateServe() error {
	...
	errs = append(errs, checkAlso("callback.also", c.Callback.Also)...)
	for i, p := range c.Participants {
		if p.Callback == nil {
			continue
		}
		...
		errs = append(errs, checkAlso(fmt.Sprintf("participants[%d].callback.also", i), p.Callback.Also)...)
	}
	...
	if c.PanelEnabled() {
		switch {
		case c.Panel.Password == "":
			errs = append(errs, errors.New("panel.password is required to serve the panel (set panel.enabled = false to turn it off)"))
		case len(c.Panel.Password) < 8:
			errs = append(errs, errors.New("panel.password must be at least 8 characters"))
		}
		// The panel owns everything under its path, so it cannot sit on top
		// of a route NHCX or an integrator calls.
		for _, reserved := range []string{"/in", "/out", "/v1", "/fhir", "/ledger", "/token", "/internal", "/healthz", "/readyz"} {
			if c.Panel.Path == reserved || strings.HasPrefix(c.Panel.Path, reserved+"/") {
				errs = append(errs, fmt.Errorf("panel.path %q is one of the adapter's own routes; use something else", c.Panel.Path))
			}
		}
	}
...
// checkAlso validates a callback's fan-out targets. where names the field
// for the error, e.g. "callback.also" or "participants[0].callback.also".
func checkAlso(where string, also []Callback) []error {
	var errs []error
	for j, extra := range also {
		if err := checkURL(extra.URL); err != nil {
			errs = append(errs, fmt.Errorf("%s[%d].url: %w", where, j, err))
		}
		for path, u := range extra.Routes {
			if err := checkURL(u); err != nil {
				errs = append(errs, fmt.Errorf("%s[%d].routes[%q]: %w", where, j, path, err))
			}
		}
		if len(extra.Also) > 0 {
			errs = append(errs, fmt.Errorf("%s[%d] may not nest an also of its own", where, j))
		}
	}
	return errs
}
...
// PanelEnabled reports whether the operator console is served. Unset means
// "on when a password is set", so the panel appears by configuring the one
// thing it cannot work without and never appears by accident.
func (c *Config) PanelEnabled() bool {
	if c.Panel.Enabled != nil {
		return *c.Panel.Enabled
	}
	return strings.TrimSpace(c.Panel.Password) != ""
}
...
func (c *Config) CallbackFor(p Participant) Callback {
	...
	if p.Callback.URL != "" {
		...
		// Routes and Also belong to the URL they were written for. A profile
		// that redirects the base without restating them would otherwise
		// scatter its traffic across two backends.
		cb.Routes = p.Callback.Routes
		cb.Also = p.Callback.Also
	}
	if p.Callback.Routes != nil {
		cb.Routes = p.Callback.Routes
	}
	if p.Callback.Also != nil {
		cb.Also = p.Callback.Also
	}
