...
// parseCfg parses a config body, failing the test if it will not parse.
...
func TestPanelDefaultsAndValidation(t *testing.T) {
	base := `{"participant": {"participantId": "1", "clientId": "c", "clientSecret": "s", "privateKey": %q},
	          "callback": {"url": "http://127.0.0.1:9/hook"}, %s}`

	// No password, no panel — and no complaint either.
	cfg := parseCfg(t, fmt.Sprintf(base, testKey, `"listen": "127.0.0.1:1"`))
	if cfg.PanelEnabled() {
		t.Error("the panel must be off until a password is set")
	}
	if cfg.Panel.Path != "/panel" || cfg.Panel.SessionHours != 12 {
		t.Errorf("panel defaults: %+v", cfg.Panel)
	}
	if err := cfg.ValidateServe(); err != nil {
		t.Errorf("a config with no panel should serve: %v", err)
	}

	// A password turns it on, and the path is normalised.
	cfg = parseCfg(t, fmt.Sprintf(base, testKey, `"panel": {"password": "long-enough", "path": "console/"}`))
	if !cfg.PanelEnabled() || cfg.Panel.Path != "/console" {
		t.Errorf("panel: %+v enabled=%v", cfg.Panel, cfg.PanelEnabled())
	}
	if err := cfg.ValidateServe(); err != nil {
		t.Errorf("valid panel rejected: %v", err)
	}

	// The ways it can be wrong: on with no password, a short password, and a
	// path that would swallow one of the adapter's own routes.
	for _, tc := range []struct{ panel, want string }{
		{`"panel": {"enabled": true}`, "panel.password is required"},
		{`"panel": {"password": "short"}`, "at least 8 characters"},
		{`"panel": {"password": "long-enough", "path": "/in"}`, "adapter's own routes"},
		{`"panel": {"password": "long-enough", "path": "/ledger/ui"}`, "adapter's own routes"},
	} {
		cfg := parseCfg(t, fmt.Sprintf(base, testKey, tc.panel))
		err := cfg.ValidateServe()
		if err == nil || !strings.Contains(err.Error(), tc.want) {
			t.Errorf("%s: want an error about %q, got %v", tc.panel, tc.want, err)
		}
	}
}
