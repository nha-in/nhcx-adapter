...
func TestLines(t *testing.T) {
	...
	for i, want := range []string{
		// The ledger id comes right after the clock: it is what "ledger show"
		// and the panel take, so a line can be traced back to its record.
		"L1 ▲ OUT v1/preauth/submit → 1000004805@hcx accepted nhcx 202 412ms 0f4c2b2e-9c7a-4d55-8a1e-2b1b0c7d9e11",
		...
		if strings.Contains(lines[i], "env=") || strings.Contains(lines[i], "ledger_id=") {
...
// The same renderer serves the server's log and "ledger follow", so a line
// built by hand from a ledger record must come out identical to the one the
// log produced for the same message.
func TestMessageRendersWithoutALedgerID(t *testing.T) {
	m := Message{Time: time.Now(), Direction: "in", Path: "v1/claim/on_submit", Peer: "2@hcx",
		Status: "delivered", PeerStatus: 200, TookMs: 12, CorrelationID: "c-1"}
	plain := ansi.ReplaceAllString(m.Render(), "")
	if !strings.Contains(plain, "▼ IN ") || !strings.Contains(plain, "callback 200") || !strings.Contains(plain, "c-1") {
		t.Errorf("line = %q", plain)
	}
	withID := ansi.ReplaceAllString(Message{Time: m.Time, ID: "7UMV0007", Direction: "in", Path: m.Path,
		Peer: m.Peer, Status: m.Status, PeerStatus: 200, TookMs: 12, CorrelationID: "c-1"}.Render(), "")
	if !strings.Contains(withID, "7UMV0007") {
		t.Errorf("an entry with an id should show it: %q", withID)
	}
	// Without an id the column is not there at all, so a ledger-less adapter
	// does not print a screenful of blank space.
	if len(plain) >= len(withID) {
		t.Errorf("the id column should only appear when there is an id:\n%q\n%q", plain, withID)
	}
}
