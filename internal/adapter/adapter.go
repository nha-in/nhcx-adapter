...
// its own callback timeout; each also target may set its own again).
type identity struct {
	prof *participant.Profile
	abdm *abdm.Client
	http *http.Client
	also []*http.Client // one per prof.Callback.Also entry, same order
}
...
func New(cfg *config.Config, logger *slog.Logger) (*Adapter, error) {
	...
	for _, prof := range profiles.All() {
		...
		for _, extra := range prof.Callback.Also {
			timeout := extra.TimeoutSeconds
			if timeout <= 0 {
				timeout = prof.Callback.TimeoutSeconds
			}
			id.also = append(id.also, &http.Client{Timeout: time.Duration(timeout) * time.Second})
		}
...
func (g *Adapter) Send(ctx context.Context, req OutboundRequest) (*OutboundResult, error) {
	...
	fail := func(err error) (*OutboundResult, error) {
		...
		// The ids the message went out under travel beside the error. A
		// connection dropped after the request was written is reported as
		// unreachable, yet NHCX can have taken the message all the same (the
		// SHA has acknowledged such sends seconds later), and only the
		// correlation id lets the caller match that answer when it comes.
		return &OutboundResult{Path: path, Headers: headers, DurationMs: entry.DurationMs, LedgerID: entry.ID}, err
	}
...
func (g *Adapter) RecordInbound(in *Inbound, res *DeliveryResult, err error) {
	...
	if res != nil {
		...
		entry.DurationMs = res.Duration.Milliseconds()
		for _, extra := range res.Also {
			peer := ledger.Peer{URL: extra.URL, StatusCode: extra.StatusCode}
			if json.Valid(extra.Body) {
				peer.Response = json.RawMessage(extra.Body)
			} else if len(extra.Body) > 0 {
				peer.Response, _ = json.Marshal(string(extra.Body))
			}
			entry.Also = append(entry.Also, peer)
		}
	}
	switch {
	case err == nil:
...
// DeliveryResult is the integrator backend's answer to a delivery. When the
// callback fans out (callback.also), Also carries the answer from each extra
// target that could be reached, in configuration order.
...
//
// With callback.also configured the same envelope goes to every target, the
// primary first. Every target is attempted even after a failure, so one
// backend being down never starves the others — but the delivery only counts
// when all of them accept, and the first failure is what NHCX is told. The
// exchange then redelivers to everyone, which is why fan-out backends must
// dedupe on x-hcx-api_call_id (they already must: NHCX redelivers on any
...
func (g *Adapter) Deliver(ctx context.Context, in *Inbound) (*DeliveryResult, error) {
	...
	res, firstErr := g.deliverTo(ctx, in, id, id.prof.Callback, id.http, body)
	for i, extra := range id.prof.Callback.Also {
		client := id.http
		if i < len(id.also) {
			client = id.also[i]
		}
		extraRes, extraErr := g.deliverTo(ctx, in, id, extra, client, body)
		if extraRes != nil {
			if res == nil {
				// The primary was unreachable; keep the extra answers on a
				// placeholder so the ledger still shows who did accept.
				res = &DeliveryResult{URL: deliveryURL(id.prof.Callback, in.Path)}
			}
			res.Also = append(res.Also, extraRes)
		}
		if firstErr == nil {
			firstErr = extraErr
		}
...
// deliverTo posts the envelope to one callback target and reads its answer.
func (g *Adapter) deliverTo(ctx context.Context, in *Inbound, id *identity, cb config.Callback, client *http.Client, body []byte) (*DeliveryResult, error) {
	target := deliveryURL(cb, in.Path)
	...
	req.Header.Set("X-Hcxkit-Type", kitType(in.Path))
...
// kitType is hcxkit's inMap spelling of the entity a path carries, which a
// backend written for the kit dispatches on. It is the protocol entity type
// with the kit's two shorter names: an insurance plan is "insurance" and a
// coverage eligibility check is "coverage". Sending the protocol spelling
// instead makes such a backend drop the message as one it does not know.
func kitType(path string) string {
	switch entity := nhcx.EntityType(path); entity {
	case "insuranceplan":
		return "insurance"
