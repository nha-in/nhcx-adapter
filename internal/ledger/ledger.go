...
type Entry struct {
	...
	// Also holds the answers from a fan-out callback's extra targets
	// (callback.also), in configuration order.
	Also []Peer `json:"also,omitempty"`
...
type Store struct {
	...
	// Subscribers watching this store from inside the process; see follow.go.
	submu sync.RWMutex
